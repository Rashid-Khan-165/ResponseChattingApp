const express = require("express");//step 1
const app = express();//step 1
const mongoose = require("mongoose");//step 4
const path =require("path"); //step 5
const Chat = require("./models/chat.js");//step 6
const methodOverride = require("method-override");//step 11



app.set("views",path.join(__dirname,"views"));//step 5
app.set("view engine","ejs"); //step 5
app.use(express.static(path.join(__dirname,"public")));
app.use(express.urlencoded({extended:true})); //Step 9 : Due to this line of code data should be parse in Create Route.
app.use(methodOverride("_method"));//step 11

main().then(()=>console.log("connection sucessful"))  // // step 4
.catch(err => console.log(err));

async function main() {
  await mongoose.connect('mongodb://127.0.0.1:27017/whatsapp'); // here whatsapp is the name of database
}

//Index Route  //step 7
app.get("/chats",async (req,res)=>{
    let chats = await Chat.find();
    console.log(chats);
    res.render("index.ejs",{chats});
});

//New Route Step 8
app.get("/chats/new",(req,res)=>{
    res.render("new.ejs");
})

//Create Route  step 9
app.post("/chats",(req,res)=>{
    let{from,to,msg} = req.body;
    let newChat = new Chat ({
        from:from,
        to:to,
        msg:msg,
        created_at: new Date(),
    })
    newChat.save().then((res)=>{
        console.log("chat was saved");
    }).catch((err)=>console.log(err));
    
    res.redirect("/chats");
});

//Edit Route step 10
app.get("/chats/:id/edit", async (req,res)=>{
    let {id} = req.params;
    let chat = await Chat.findById(id);
    res.render("edit.ejs",{chat});
})

//Update Route  step 11
app.put("chats/:id", async (req,res)=>{
    let {id} = req.params;
    let {msg : newMsg} = req.body;
    let updatedChat = await  Chat.findByIdAndUpdate(
        id,
        {msg:newMsg},
        {runValidators:true,new:true}
    ); 
    res.redirect("/chats");
})

//Delete Route step 12
app.delete("/chats/:id",async (req,res)=>{
    let {id} = req.params;
    let deletedChat = await Chat.findByIdAndDelete(id);
    console.log(deletedChat);
    res.redirect("/chats");
})


app.get("/",(req,res)=>{  //step 3  for checking root path is working
    res.send("root is Working")
})

app.listen(8080,()=>{ //step 2
    console.log("Server is listening on port 8080.")
});